...
# ==================== 配置 ====================
MODEL_FILE  = "ball_yolo26_single_int8_split.rknn"
CAMERA_ID   = 0
IMG_SIZE    = 640
CONF_THRESH = 0.5
UART_PORT   = "/dev/ttyS1"
...
def postprocess(output, scale, xo, yo, iw, ih):
    # Split-output single-class YOLO26: [batch, 300, 5].
    # Each row is [x1, y1, x2, y2, confidence]; class_id is always 0.
    data = np.asarray(output)
    if data.shape == (1, 5, 300):
        data = data.transpose(0, 2, 1)
    if data.shape != (1, 300, 5):
...
def inference_thread(worker_id, core_mask):
    ...
    while running:
        ...
        if not outputs:
            ...
            break
        if len(outputs) != 2:
            print(f"[NPU-{worker_id}] Expected 2 outputs, got {len(outputs)}")
            stop(f"NPU-{worker_id} invalid output count")
            break
        boxes_out = np.asarray(outputs[0])
        confidence_out = np.asarray(outputs[1])
        if boxes_out.shape == (1, 4, 300):
            boxes_out = boxes_out.transpose(0, 2, 1)
        if confidence_out.shape == (1, 1, 300):
            confidence_out = confidence_out.transpose(0, 2, 1)
        if boxes_out.shape != (1, 300, 4):
            print(
                f"[NPU-{worker_id}] Unexpected boxes shape: "
                f"{boxes_out.shape}"
            )
            stop(f"NPU-{worker_id} invalid boxes output shape")
            break
        if confidence_out.shape != (1, 300, 1):
            print(
                f"[NPU-{worker_id}] Unexpected confidence shape: "
                f"{confidence_out.shape}"
            )
            stop(f"NPU-{worker_id} invalid confidence output shape")
            break
        out = np.concatenate((boxes_out, confidence_out), axis=2)
        max_conf = float(np.max(confidence_out))
