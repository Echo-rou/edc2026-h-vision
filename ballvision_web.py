...
.toolbar{display:flex;align-items:center;justify-content:center;gap:10px;flex-wrap:wrap;padding-top:15px}
button{height:40px;border:1px solid #444b53;background:#272c32;color:#fff;padding:0 16px;border-radius:6px;font-size:15px;cursor:pointer}
button:hover{background:#333941}button.primary{background:#c93838;border-color:#dc4a4a}button.primary:hover{background:#dd4141}button:disabled{opacity:.45;cursor:default}
.timer{min-width:85px;font-variant-numeric:tabular-nums;color:#f1c75b;text-align:center}.hint{text-align:center;color:#9099a3;font-size:13px;margin-top:10px}
</style>
</head>
<body>
...
<main>
  ...
  <div class="toolbar">
    <button id="record" class="primary">开始录像</button>
    <button id="stop" disabled>停止并保存</button>
    <button id="snapshot">保存截图</button>
    <button id="fullscreen">全屏</button>
    <span id="timer" class="timer">00:00.0</span>
  </div>
  <div class="hint">录像保存在电脑浏览器的下载目录。测试前点击“开始录像”，结束后点击“停止并保存”。</div>
</main>
<script>
const source=document.getElementById('source'),canvas=document.getElementById('view'),ctx=canvas.getContext('2d');
const recordBtn=document.getElementById('record'),stopBtn=document.getElementById('stop'),timer=document.getElementById('timer');
...
recordBtn.onclick=()=>{const stream=canvas.captureStream(25);const type=mime();chunks=[];recorder=new MediaRecorder(stream,type?{mimeType:type,videoBitsPerSecond:5000000}:undefined);recorder.ondataavailable=e=>{if(e.data.size)chunks.push(e.data)};recorder.onstop=()=>{const blob=new Blob(chunks,{type:recorder.mimeType});const ext=recorder.mimeType.includes('mp4')?'mp4':'webm';const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='ball_test_'+new Date().toISOString().replace(/[:.]/g,'-')+'.'+ext;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)};recorder.start(1000);started=performance.now();timerId=setInterval(()=>timer.textContent=format(performance.now()-started),100);recordBtn.disabled=true;stopBtn.disabled=false;document.getElementById('state').textContent='录像中'};
stopBtn.onclick=()=>{if(recorder&&recorder.state!=='inactive')recorder.stop();clearInterval(timerId);recordBtn.disabled=false;stopBtn.disabled=true;document.getElementById('state').textContent='实时画面'};
document.getElementById('snapshot').onclick=()=>canvas.toBlob(blob=>{const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='ball_snapshot_'+Date.now()+'.jpg';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)},'image/jpeg',.92);
document.getElementById('fullscreen').onclick=()=>document.querySelector('.video').requestFullscreen();
</script>
</body></html>"""
...
def parse_args():
    ...
    parser.add_argument("--model", default="/home/orangepi/ball_best_int8_split.rknn")
    parser.add_argument("--host", default="0.0.0.0")
    parser.add_argument("--port", type=int, default=8080)
    parser.add_argument("--address", default="192.168.5.1")
